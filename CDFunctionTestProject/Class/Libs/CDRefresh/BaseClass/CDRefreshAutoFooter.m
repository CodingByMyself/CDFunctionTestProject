...
@implementation CDRefreshAutoFooter
...
- (void)scrollViewContentOffsetDidChange:(NSDictionary *)change
{
    ...
    if (self.state != CDRefreshStateIdle || !self.automaticallyRefresh || self.mj_y == 0) return;
...
- (void)scrollViewPanStateDidChange:(NSDictionary *)change
{
    ...
    if (self.state != CDRefreshStateIdle) return;
...
- (void)setState:(CDRefreshState)state
{
    ...
    if (state == CDRefreshStateRefreshing) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self executeRefreshingCallback];
        });
    } else if (state == CDRefreshStateNoMoreData || state == CDRefreshStateIdle) {
